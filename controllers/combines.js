		...
		if ( results && results.length ) {
			...
			const players_query = `
				SELECT 
					...
					t.name,t.effective_mmr,t.wins,t.losses,t.tier,NULL as effective_tier,
					pu.nickname
				FROM combine_match_players AS p
			...
			req.db.query(players_query, [ SEASON, league, match_id ], (err, results) => {
				...
				if ( results && results.length ) {
					for ( let i = 0; i < results.length; ++i ) {
						const p = results[i];
						p.effective_tier = getTierFromMMR(p.end_mmr ? p.end_mmr : p.start_mmr);
...
router.get('/combines/cup', (req, res) => {
	...
	const query = `
		SELECT 
			...
			(t.wins+t.losses) as games, format(((t.wins+t.losses) / 3),0) AS series,
			(t.wins*0.5) AS win_points, ((t.wins+t.losses)/3) as series_points,
			((t.wins*0.5) + ((t.wins+t.losses)/3)) as points,
			format(((t.wins*0.5) + ((t.wins+t.losses)/3)),1) as nice_points
		FROM tiermaker AS t 
		WHERE t.season = ?
